...
def addBits(pBit: Boolean, qBit: Boolean, carryBit: Boolean): Boolean = {
  if (pBit.equals(true) && qBit.equals(true))
    carryBit.equals(true)
  else
    false
}
...
def convertIntListToBooleanList(intList: List[Int]) = {
  intList.map(listelement => if (listelement == 1) true else false)
}
...
def convertBooleanListToIntList(booleanList: List[Boolean]) = {
  booleanList.map(listelement => if (listelement.equals(true)) 1 else 0)
}
